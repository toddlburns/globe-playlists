...
const genreLocations = {
  ...
  'sad sierreño': { lat: 25.6866, lng: -100.3161, location: 'Monterrey, Mexico' },
  'sierreño': { lat: 25.6866, lng: -100.3161, location: 'Monterrey, Mexico' },
  ...
  'dinner party': { lat: 34.0522, lng: -118.2437, location: 'Los Angeles, USA' },
  'summer': { lat: 34.0522, lng: -118.2437, location: 'Los Angeles, USA' },
  'sunday': { lat: 34.0522, lng: -118.2437, location: 'Los Angeles, USA' },
  'sad songs': { lat: 51.5074, lng: -0.1278, location: 'London, UK' },
};

const defaultLocation = { lat: 51.5074, lng: -0.1278, location: 'London, UK' };

// Read the corrected CSV
const csv = fs.readFileSync('./corrected-playlists.csv', 'utf-8');
...
lines.forEach(line => {
  ...
  const name = match[1];
  const url = match[2];
  const spotifyId = match[3];

  if (name === 'COULD NOT FETCH') return;
...
console.log(`Regenerated playlists-data.js with ${playlists.length} correctly named playlists`);
